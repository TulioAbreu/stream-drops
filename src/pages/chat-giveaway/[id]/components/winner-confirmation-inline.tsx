...
export function WinnerConfirmationInline({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const diff = Math.floor((Date.now() - confirmationStartTime.getTime()) / 1000);
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div
            className={cn(
              ...
              isExpanded ? "w-0 h-8 opacity-0 scale-75" : "w-8 h-8 opacity-100 scale-100"
            )}
          >
            {rank}
          </div>
                  ...
                  {pendingWinner.subscriptionMonths != null &&
                    pendingWinner.subscriptionMonths > 0 && (
                      <Badge variant="secondary" className="text-[10px] px-1.5 py-0">
                        ...
                        {pendingWinner.subscriptionMonths === 1 ? "mês" : "meses"}
                      </Badge>
                    )}
                  <span
